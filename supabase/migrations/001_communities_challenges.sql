...
-- Any authenticated user can insert a community or challenge
create policy "communities: auth insert"  on communities  for insert with check (auth.role() = 'authenticated');
create policy "challenges: auth insert"   on challenges   for insert with check (auth.role() = 'authenticated');
